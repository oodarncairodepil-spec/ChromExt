...
SELECT COUNT(*) FROM quick_reply_templates;
  ...
  const fs = require('fs');
  fs.writeFileSync('/Users/plugoemployee/ChromExt/database/EMERGENCY-SQL-FIX.sql', sqlCommands);
  console.log('\n📄 Commands saved to: database/EMERGENCY-SQL-FIX.sql');
}
...
async function main() {
  ...
  console.log('2. Copy commands from database/EMERGENCY-SQL-FIX.sql');
  console.log('3. Execute each command ONE BY ONE');
  console.log('4. Run: node database/verify-rls-fix.js');

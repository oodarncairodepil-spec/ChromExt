...
async function checkCurrentState() {
  ...
  try {
    ...
    if (hasLegacyData) {
      ...
      console.log('   File: supabase/migrations/migrate-logo-data-to-text.sql');
    ...
    if (error.message.includes('column "logo_data" does not exist')) {
      ...
      console.log('   Please run: supabase/migrations/add-courier-logo-column.sql first');
...
async function performSimpleMigration() {
  ...
  try {
    ...
    console.log('2. Run the contents of: supabase/migrations/migrate-logo-data-to-text.sql');
